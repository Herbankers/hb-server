            ...
            while isDatabase:
                ...
                if keuze == 0:
                    ...
                    try:
                        try:
                            ...
                            cursor.execute("""SELECT users.user_id, first_name, last_name, accounts.iban, type, balance, card_id, pin, attempts FROM users JOIN registrations ON users.user_id = registrations.user_id JOIN accounts ON accounts.iban = registrations.iban JOIN cards ON cards.iban = accounts.iban WHERE users.user_id = '%s'""" % (db_gebruikerid))
                ...
                if keuze == 1:
                    ...
                    try:
                        ...
                        try:
                            cursor.execute("SELECT user_id FROM users WHERE first_name = %s AND last_name = %s",(db_first_name, db_last_name))
                        except (mysql.connector.Error, mysql.connector.Warning) as e:
                            print(e)
                        ...
                        try:
                            cursor.execute("INSERT INTO accounts (iban,type,balance) VALUES (%s,%s,%s)",(db_iban,db_type,db_balance))
                        except (mysql.connector.Error, mysql.connector.Warning) as e:
                            print(e)
                       ...
                       # Query voor maken van de bankpas
                        try:
                            cursor.execute("INSERT INTO cards (card_id,iban,user_id,pin) VALUES (%s,%s,%s,%s)",(db_card_id, db_iban,db_user_id,db_pin))
                        except (mysql.connector.Error, mysql.connector.Warning) as e:
                            print(e)
                    ...
                    try:
                        try:
                            ...
                            cursor.execute( """DELETE FROM gebruikers WHERE id = '%s'""" % (db_gebruikerid))
                        except (mysql.connector.Error, mysql.connector.Warning) as e:
                            print(e)
                        try:
                            results = cursor.fetchone()
